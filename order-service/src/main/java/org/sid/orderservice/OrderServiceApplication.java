...
public class OrderServiceApplication {
	...
	CommandLineRunner start(OrderRepository orderRepository,
		...
		return args -> {
			...
			for (int i = 0; i <20 ; i++) {
				Order order=Order.builder().customerId(customers.get(random.nextInt(customers.size())).getId()).status(Math.random()>0.5?
				OrderStatus.PENDING:OrderStatus.CANCELED).createdAt(new Date()).build();
